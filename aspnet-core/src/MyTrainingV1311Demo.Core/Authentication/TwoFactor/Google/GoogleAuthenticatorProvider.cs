...
namespace MyTrainingV1311Demo.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : MyTrainingV1311DemoServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("MyTrainingV1311Demo", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
